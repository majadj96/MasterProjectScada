					...
					case SupportedProfiles.PowerTransformer:
						{
							...
							TransformAndLoadReport report = ProjekatMaster2019Importer.Instance.CreateNMSDelta(concreteModel);

							if (report.Success)
							{
								nmsDelta = ProjekatMaster2019Importer.Instance.NMSDelta;
								success = true;
							}
							else
							{
							...
							ProjekatMaster2019Importer.Instance.Reset();

							break;
						}
                    case SupportedProfiles.ProjekatMaster2019:
                        {
                            // transformation to DMS delta					
                            TransformAndLoadReport report = ProjekatMaster2019Importer.Instance.CreateNMSDelta(concreteModel);
