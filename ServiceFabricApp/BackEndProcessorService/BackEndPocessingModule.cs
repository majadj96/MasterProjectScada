...
namespace BackEndProcessorService
{
    public class BackEndPocessingModule : IBackendProcessor
    {
        ...
        public BackEndPocessingModule(IPointUpdateService pointUpdateProxy, IAlarmService alarmServiceProxy, IPub publisherProxy, IMeasurementRepository measurementRepository)
        {
            this.alarmServiceProxy = alarmServiceProxy;
        ...
        public void Process(ProcessingObject[] inputObj)
        {
            ...
            for (int index = 0; index < inputObj.Length; index++)
            {
                ...
                if (inputObj[index].PointType == PointType.ANALOG_INPUT_16 || inputObj[index].PointType == PointType.ANALOG_OUTPUT_16)
                {
                    ...
                    Measurement newMeas = new Measurement(DateTime.UtcNow) { Gid = inputObj[index].Gid, ChangedTime = DateTime.Now, Value = (int)((AnalogPoint)(inputObj[index])).EguValue };
                    measurementsDB.Add(newMeas);
                }
                else
                {
                    measurement.Add(new ScadaUIExchangeModel()
                    ...
                    Measurement newMeas = new Measurement(DateTime.UtcNow) { Gid = inputObj[index].Gid, ChangedTime = inputObj[index].Timestamp, Value = (int)inputObj[index].RawValue };
                    measurementsDB.Add(newMeas);
                }
            }
        ...
        private void InitializeProcessingModules()
        {
            ...
            this.ProcessingPipeline.Add(new AlarmingModule(alarmServiceProxy));
