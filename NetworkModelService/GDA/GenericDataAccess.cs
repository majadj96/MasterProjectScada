...
namespace NetworkModelService.GDA
{
    public class GenericDataAccess : INetworkModelGDAContract, IModelUpdateContract
    {
        ...
        public List<long> GetAllGIDs()
        {
            try
            {
                return nm.RetrieveAllGIDs();
            }
            catch (Exception ex)
            {
                string message = string.Format("Retrieving GIDs failed. Reason: {0}", ex.Message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                throw new Exception(message);
            }
        }

        public List<ModelCode> GetAllProps(long gid)
        {
            List<ModelCode> retVal = new List<ModelCode>();

            try
            {
                retVal = nm.RetrieveAllProps(gid);
            }
            catch (Exception ex)
            {
                string message = string.Format("Getting properties failed. Reason: {0}", ex.Message);
                CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                throw new Exception(message);
            }
